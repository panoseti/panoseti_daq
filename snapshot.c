...
// Generates a fixed-size JSON string for a single-quabo PH snapshot.
...
// Generates a fixed-size JSON string for a four-quabo image snapshot.
...
void WritePHSnapshotsToUds(PACKET_HEADER *header, uint8_t *data) {
    ...
    int json_len = sprint_ph_snapshot_json(json_buffer, sizeof(json_buffer), header);
    if (json_len > 0) {
...
void WriteImgSnapshotsToUds(PACKET_HEADER *header, uint8_t *data) {
    ...
    int json_len = sprint_img_snapshot_json(json_buffer, sizeof(json_buffer), header);
    if (json_len > 0) {
